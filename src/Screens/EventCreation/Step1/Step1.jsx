...
const Step1 = ({ onNext, defaultValues }) => {
  const {
    ...
    defaultValues: {
      inviteType: 'Please Select',
      occasion: 'Please Select',
      ...defaultValues,
    },
  });

  const inviteTypeValue = watch('inviteType');
  const isOccasionDisabled =
    !inviteTypeValue || inviteTypeValue === 'Please Select';
  ...
  return (
    <StepLayout CurrentStep={0}>
      <div className="flex flex-col justify-center items-center pt-10 pb-16 min-[1850px]:pb-0 min-[1850px]:pt-10 gap-9 2xl:gap-12">
        <h1 className="font-Inter text-[32px] 2xl:text-[38px] font-[500] text-center">
          I want to invite people to
        </h1>

        <form
          onSubmit={handleSubmit(Step1Val)}
          className="flex flex-col gap-8 2xl:gap-11 justify-center"
          noValidate
        >
          {/* First Select */}
          <div className="relative flex flex-col justify-center">
            <select
              ...
              className={`bg-white appearance-none text-[#A0A0A0] font-Inter text-[14px] 2xl:text-[16px] border rounded-[15px] border-[#000] focus:outline-none px-5 py-3 2xl:py-4 w-[350px] 2xl:w-[400px] cursor-pointer ${
                errors.inviteType ? 'border-red-500 ' : ''
              }`}
            >
            ...
            <ArrowDown
              className="pointer-events-none absolute top-3 right-3 size-6 2xl:size-8 text-[#FFB5C0]"
              strokeWidth={3}
            />
            {errors.inviteType && (
              <p className="text-red-500 text-[13px] mt-2 -mb-2 font-Inter">
          ...
          <div className="relative flex flex-col gap-2">
            ...
            <ArrowDown
              className={`pointer-events-none absolute top-11 2xl:top-[50px] right-3 size-6 2xl:size-8 ${
                isOccasionDisabled ? 'text-[#E0E0E0]' : 'text-[#FFB5C0]'
              }`}
              strokeWidth={3}
            />
